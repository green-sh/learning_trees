...
class RegressionTree:
    ...
    def train(self, x, y, max_depth=5, min_elements=1, min_samples=2):
        best_score = np.infty
        ...
        if len(np.unique(x_left)) <= min_elements or x_left.shape[1] < min_samples:
            self.left = ValueNode(best_left_prediction, graph=self.graph, parent_name=str(id(self)))
        else:
            self.left = RegressionTree(graph=self.graph, parent_name=str(id(self))).train(x_left, y_left, max_depth=max_depth - 1, min_elements=min_elements, min_samples=min_samples)
            
        if len(np.unique(x_right)) <= min_elements or x_right.shape[1] < min_samples:
            self.right = ValueNode(best_right_prediction, graph=self.graph, parent_name=str(id(self)))
        else:
            self.right = RegressionTree(graph=self.graph, parent_name=str(id(self))).train(x_right, y_right, max_depth=max_depth - 1, min_elements=min_elements, min_samples=min_samples)
